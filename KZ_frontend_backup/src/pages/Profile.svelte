<script lang="ts">
  ...
  import { 
    Edit3, X, Save, KeyRound, ImagePlus, 
    Loader2, Mail, ShieldAlert, Eye, EyeOff, GraduationCap
  } from 'lucide-svelte';
    ...
    <div class="body-card" in:fly={{ y: 40, delay: 100, duration: 450, easing: cubicOut }}>
        ...
        <div class="edit-form" in:fade={{ duration: 300 }}>
          ...
          <div class="form-grid">
            ...
            <div class="input-group full-width">
              <label for="bio">نبذة عن خبراتك العلمية</label>
              <textarea id="bio" bind:value={formData.bio} disabled={isSaving} class="sv-textarea" placeholder="اكتب عن مجالات تخصصك وخبراتك التدريسية..."></textarea>
            </div>
          </div>
      ...
      {:else}
        <div class="read-mode" in:fade={{ duration: 300 }}>
          ...
          <div class="bio-box">
            <p class="bio-text">
              {$user?.bio || 'لم يتم إضافة نبذة بعد، ابدأ بكتابة خبراتك ومجالات تخصصك العلمية.'}
            </p>
          </div>
        </div>
      {/if}
...
<style>
  ...
  /* خلفية أغمق قليلاً لتمييز المعلم */
  .header-cover { height: 180px; background: linear-gradient(135deg, #1f3047 0%, var(--dark) 60%, var(--primary) 100%); position: relative; overflow: hidden; }
  ...
  /* باج المعلم الجديد */
  .profile-badge { display: inline-flex; align-items: center; gap: 0.3rem; background: rgba(255,255,255, 0.15); color: #e2e8f0; border: 1px solid rgba(255,255,255,0.2); padding: 0.3rem 0.8rem; border-radius: 99px; font-size: 0.75rem; font-weight: 800; width: fit-content; margin-bottom: 0.5rem; }
